...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-5 fade-in">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Daftar seluruh dokumen yang pernah diajukan.</p>
        </div>
        <a href="{{ route('madrasah.pengajuan.index') }}" class="btn-xs btn-primary-xs">
    ...
    <div class="bg-white rounded-xl border border-gray-200 px-4 py-3 mb-4 fade-in">
        <form method="GET" class="flex items-center gap-2">
            <select name="status" onchange="this.form.submit()" class="border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-600 focus:outline-none">
                <option value="">Semua status</option>
                <option value="pending"  {{ request('status') === 'pending'  ? 'selected' : '' }}>Menunggu</option>
                <option value="diterima" {{ request('status') === 'diterima' ? 'selected' : '' }}>Diterima</option>
                <option value="ditolak"  {{ request('status') === 'ditolak'  ? 'selected' : '' }}>Ditolak</option>
            </select>
            @if(request('status'))<a href="{{ route('madrasah.pengajuan.riwayat') }}" class="text-xs text-gray-400 hover:text-gray-600">Reset</a>@endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden fade-in">
        ...
        <div class="p-10 text-center">
            ...
            <p class="text-xs text-gray-400 mt-1">Pengajuan dokumen madrasah akan muncul di sini.</p>
        </div>
        @else
        {{-- Desktop: tabel --}}
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="badge {{ $p->status === 'diterima' ? 'badge-green' : ($p->status === 'ditolak' ? 'badge-red' : 'badge-yellow') }}">{{ $p->status_label }}</span></td>
                    <td style="text-align:right">
                        <div class="flex items-center justify-end gap-1">
                            <a href="{{ route('madrasah.pengajuan.show', $p->id) }}" class="btn-icon blue" title="Lihat Detail">
                            ...
                            @if($p->status === 'pending')
                            <form action="{{ route('madrasah.pengajuan.batalkan', $p->id) }}" method="POST"
        ...
        <div class="px-4 py-3" style="border-top:1px solid #f5f5f5">{{ $riwayat->links() }}</div>
        </div>
        ...
        <div class="md:hidden divide-y divide-gray-50">
            ...
            <div class="p-4">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('madrasah.pengajuan.show', $p->id) }}"
