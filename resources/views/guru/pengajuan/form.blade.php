...
<div class="max-w-2xl mx-auto">
    ...
    <a href="{{ route('guru.pengajuan.index') }}" class="inline-flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 mb-3">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div class="px-5 py-4 border-b border-gray-100">
            ...
            @if($jenis->deskripsi)
            <p class="text-sm text-gray-500 mt-1">{{ $jenis->deskripsi }}</p>
            @endif
        </div>
        ...
        {{-- Error duplikat / lainnya --}}
        @if(session('error'))
        <div class="px-5 pt-4">
            <div class="bg-red-50 border border-red-100 text-red-700 rounded-lg px-4 py-3 flex items-start gap-2.5 text-sm">
        ...
        <form action="{{ route('guru.pengajuan.store', $jenis->id) }}" method="POST" enctype="multipart/form-data" id="form-ajuan-guru" class="p-5 space-y-4">
            @csrf

            {{-- Data pemohon — auto-filled, read-only --}}
            <div class="bg-gray-50 rounded-lg p-3.5 space-y-2.5">
                <p class="text-2xs font-medium text-gray-400 uppercase tracking-wide">Data Pemohon (otomatis dari akun Anda)</p>
                <div class="grid sm:grid-cols-2 gap-2.5 text-sm">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <p class="font-medium text-gray-800">{{ $guru->nama_madrasah }}</p>
                    </div>
                    <div>
                ...
                <a href="{{ route('guru.profil.form') }}" class="inline-flex items-center gap-1 text-2xs text-primary-700 hover:text-primary-800 font-medium">
            ...
            <div>
                ...
                <input type="text" name="token" value="{{ old('token') }}" maxlength="6"
                       class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm uppercase tracking-wider font-mono focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-400 @error('token') border-red-300 @enderror"
                       placeholder="ABCDEF">
            ...
            <button type="submit" id="btn-submit-ajuan"
...
<script>
    ...
    document.getElementById('form-ajuan-guru')?.addEventListener('submit', function (e) {
        const inputs = this.querySelectorAll('[required]');
        let valid = true;
        inputs.forEach(i => { if (!i.value.trim()) valid = false; });
        if (!valid) return;

        const btn = document.getElementById('btn-submit-ajuan');
        btn.disabled = true;
