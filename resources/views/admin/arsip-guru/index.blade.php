...
@section('title', 'Arsip Digital Guru')
...
<div class="flex items-center justify-between mb-5 fade-in">
    <div>
        <h1 class="text-base font-semibold text-zinc-900">Arsip Digital Guru</h1>
        <p class="text-xs text-zinc-400 mt-0.5">Pantau dan verifikasi arsip yang diunggah guru madrasah</p>
    </div>
    <div class="flex items-center gap-2">
        <a href="{{ route('admin.arsip-guru.export', request()->query()) }}"
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5 fade-in">
    ...
    <div class="bg-white rounded-xl border border-zinc-100 px-4 py-4">
        <p class="text-2xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Guru Upload</p>
        <div class="flex items-end justify-between">
...
<div class="bg-white rounded-xl border border-zinc-100 px-4 py-3 mb-4 fade-in">
    <form method="GET" class="flex flex-wrap gap-2 items-center">
        <div class="relative flex-1 min-w-44">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Judul dokumen, nama guru, PegID..."
        ...
        @if(request()->hasAny(['search','kategori_id','madrasah_id','status','tahun']))
        <a href="{{ route('admin.arsip-guru.index') }}" class="text-xs text-zinc-400 hover:text-zinc-600 transition">Reset</a>
        @endif
    </form>
</div>
...
<div class="bg-white rounded-xl border border-zinc-100 overflow-hidden fade-in"
    ...
    <div x-show="selected.length > 0" x-cloak x-transition
        ...
        <form method="POST" action="{{ route('admin.arsip-guru.bulk-verify') }}"
    ...
    <table class="w-full text-sm">
        <thead>
            <tr style="border-bottom:1px solid #f4f4f5">
                ...
                <th class="px-4 py-2.5 text-left text-2xs font-semibold text-zinc-600 uppercase tracking-wider hidden md:table-cell">Guru</th>
        ...
        <tbody>
            ...
            <tr class="tbl-row" style="border-bottom:1px solid #fafafa" x-bind:class="selected.includes('{{ $item->id }}') ? 'bg-primary-50/40' : ''">

                {{-- Checkbox --}}
                <td class="px-4 py-3">
                    <input type="checkbox" class="row-check accent-primary-600 w-3.5 h-3.5 rounded"
                        value="{{ $item->id }}"
                        @change="selected.includes('{{ $item->id }}')
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="min-w-0">
                            <p class="text-xs font-medium text-zinc-800 truncate max-w-48">{{ $item->judul }}</p>
                            <p class="text-2xs text-zinc-400">{{ $item->kategori->nama ?? '—' }}</p>
                        </div>
                    </div>
                </td>
                ...
                <td class="px-4 py-3 hidden md:table-cell">
                    <p class="text-xs font-medium text-zinc-800">{{ $item->guru->nama }}</p>
                    <code class="text-2xs bg-zinc-100 text-zinc-600 px-1.5 py-0.5 rounded font-mono">{{ $item->guru->pegid }}</code>
                </td>

                {{-- Madrasah --}}
                <td class="px-4 py-3 hidden lg:table-cell">
                    <span class="text-xs text-zinc-500">{{ $item->guru->madrasah?->nama_madrasah ?? '—' }}</span>
                </td>

                {{-- Tahun --}}
                <td class="px-4 py-3 text-center hidden sm:table-cell">
                    <span class="text-xs text-zinc-500 font-mono">{{ $item->tahun ?? '—' }}</span>
                </td>
                ...
                <td class="px-4 py-3 text-center">
                    ...
                    <p class="text-2xs text-zinc-400 mt-1 max-w-24 truncate mx-auto" title="{{ $item->catatan_admin }}">
                        {{ $item->catatan_admin }}
                    </p>
                    @endif
                </td>
                ...
                <td class="px-4 py-3" x-data="{ catatanModal: false, catatan: '{{ addslashes($item->catatan_admin ?? '') }}' }">
                    <div class="flex items-center justify-center gap-0.5">
                        ...
                        {{-- Lihat arsip guru ini --}}
                        <a href="{{ route('admin.arsip-guru.show', $item->guru_id) }}" class="btn-icon" title="Semua Arsip Guru Ini">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        </a>
                        {{-- Verifikasi / Batalkan --}}
                        @if(!$item->is_verified)
                        <form method="POST" action="{{ route('admin.arsip-guru.verify', $item->id) }}" class="inline">
                            @csrf @method('PATCH')
                        ...
                        @else
                        <form method="POST" action="{{ route('admin.arsip-guru.unverify', $item->id) }}" class="inline">
                            @csrf @method('PATCH')
                        ...
                        <form method="POST" action="{{ route('admin.arsip-guru.destroy', $item->id) }}"
                              data-confirm="Hapus arsip ini?\n\n{{ addslashes($item->judul) }}\nMilik: {{ addslashes($item->guru->nama) }}" data-confirm-btn="Ya, Hapus" class="inline">
                            @csrf @method('DELETE')
                    ...
                    <div x-show="catatanModal" x-cloak
                        ...
                        <div class="bg-white rounded-xl w-full max-w-sm p-5" style="box-shadow:0 8px 24px rgba(0,0,0,0.15)" @click.stop>
                            ...
                            <p class="text-2xs text-zinc-400 mb-3">Untuk arsip: <strong>{{ $item->judul }}</strong></p>
            ...
            <tr>
                <td colspan="7" class="py-14 text-center">
                    ...
                    <p class="text-xs text-zinc-400">Belum ada arsip ditemukan</p>
                    <p class="text-2xs text-zinc-300 mt-1">Coba ubah filter atau tunggu guru mengunggah arsip</p>
                </td>
            </tr>
            @endforelse
        </tbody>
