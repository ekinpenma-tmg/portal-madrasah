...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-5">
        ...
        <p class="text-sm text-gray-500 mt-0.5">Pilih jenis dokumen yang ingin Anda ajukan ke Penma.</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-10 text-center">
        ...
        <p class="text-sm font-medium text-gray-700">Belum ada jenis dokumen tersedia</p>
        <p class="text-xs text-gray-400 mt-1">Hubungi admin Penma untuk informasi lebih lanjut.</p>
    </div>
    @else
    <div class="grid sm:grid-cols-2 gap-3">
        @foreach($jenisDokumen as $jenis)
        <a href="{{ route('guru.pengajuan.form', $jenis->id) }}"
           class="group bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-3 hover:border-primary-300 hover:shadow-sm transition">
            ...
            <div class="min-w-0">
                ...
                @if($jenis->deskripsi)
                <p class="text-xs text-gray-400 truncate">{{ $jenis->deskripsi }}</p>
                @endif
            </div>
            <svg class="w-4 h-4 text-gray-300 ml-auto flex-shrink-0 group-hover:text-primary-500 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
