...
<div class="bg-white rounded-xl border border-gray-200 px-4 py-3 mb-4 flex items-center justify-between fade-in">
    <div class="flex items-center gap-3">
        ...
        <div>
            <p class="text-sm font-semibold text-gray-900">{{ $guru->nama }}</p>
            <p class="text-xs text-gray-400 mt-0.5">
                PegID: <span class="font-mono">{{ $guru->pegid }}</span>
                @if($guru->madrasah) · {{ $guru->madrasah->nama_madrasah }} @endif
            </p>
        </div>
    </div>
    <span class="badge {{ $guru->is_active ? 'badge-green' : 'badge-gray' }}">
        <span class="w-1.5 h-1.5 rounded-full inline-block mr-1 {{ $guru->is_active ? 'bg-green-600' : 'bg-gray-400' }}"></span>
        {{ $guru->is_active ? 'Akun aktif' : 'Nonaktif' }}
    </span>
</div>

{{-- 3 Stat utama --}}
<div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4 fade-in">
    <a href="{{ route('guru.arsip.index') }}" class="stat-card group block transition hover:shadow-md hover:-translate-y-0.5">
    ...
    <a href="{{ route('guru.arsip.index', ['verified' => 1]) }}" class="stat-card group block transition hover:shadow-md hover:-translate-y-0.5">
        <div class="flex items-center justify-between">
            <div>
    ...
    <a href="{{ route('guru.pengajuan.riwayat', ['status' => 'pending']) }}" class="stat-card amber group block transition hover:shadow-md hover:-translate-y-0.5">
        <div class="flex items-center justify-between">
            <div>
...
{{-- 2 kolom: kelengkapan arsip, pengajuan terakhir --}}
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4 fade-in">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div class="flex items-center gap-2 py-1.5 border-b border-gray-50 last:border-0">
            <span class="w-2 h-2 rounded-full flex-shrink-0 {{ $item['terisi'] ? 'bg-primary-700' : 'bg-gray-300' }}"></span>
        ...
        <div class="mt-3 bg-gray-100 rounded-full h-1.5 overflow-hidden">
            <div class="h-full bg-primary-700 rounded-full transition-all" style="width: {{ $persen }}%"></div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4">
        <div class="flex items-center justify-between mb-3">
            ...
            <a href="{{ route('guru.pengajuan.riwayat') }}" class="text-xs text-primary-700 font-medium hover:underline">Lihat semua</a>
        </div>
        ...
        <div class="text-center py-6">
            ...
            <a href="{{ route('guru.pengajuan.index') }}" class="inline-flex items-center gap-1.5 mt-2 btn-xs btn-primary-xs">
                Ajukan Dokumen
            </a>
        </div>
        @endforelse
