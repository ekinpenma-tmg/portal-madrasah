...
<div class="max-w-xl">
    ...
    <div class="flex items-center gap-2.5 mb-4 fade-in">
        <a href="{{ route('admin.download.index') }}" class="btn-icon">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <div>
            <h1 class="text-sm font-semibold text-zinc-900">Tambah File Download</h1>
            <p class="text-xs text-zinc-400 mt-0.5">Unggah dokumen baru untuk diunduh publik</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.download.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="kategori" value="{{ old('kategori') }}"
                       ...
                       placeholder="Contoh: Perizinan, Akreditasi">
            </div>
            <div>
                <label class="block text-xs font-medium text-zinc-600 mb-1.5">Deskripsi</label>
                <textarea name="deskripsi" rows="3" class="w-full border border-zinc-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-zinc-300 resize-none" placeholder="Keterangan singkat tentang file ini">{{ old('deskripsi') }}</textarea>
            </div>
            <div>
                <label class="block text-xs font-medium text-zinc-600 mb-1.5">Upload File <span class="text-red-500">*</span></label>
                ...
                <p class="text-zinc-400 text-2xs mt-1">Maksimal 10MB</p>
                @error('file')<p class="text-red-500 text-2xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div class="flex gap-2 pt-2" style="border-top:1px solid #f4f4f5">
                ...
                <a href="{{ route('admin.download.index') }}" class="text-xs font-medium text-zinc-500 hover:text-zinc-700 px-4 py-2 rounded-lg hover:bg-zinc-100 transition">
                    Batal
                </a>
            </div>
        </form>
