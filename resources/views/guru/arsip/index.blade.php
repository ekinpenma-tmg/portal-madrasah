...
<div class="flex items-center justify-between mb-5 fade-in">
    <div>
        <h1 class="text-lg font-semibold text-gray-900">Arsip Saya</h1>
        <p class="text-sm text-gray-500 mt-0.5">Semua dokumen yang sudah Anda unggah</p>
    </div>
    <a href="{{ route('guru.arsip.create') }}" class="btn-xs btn-primary-xs">
...
<div class="bg-white rounded-xl border border-gray-200 px-4 py-3 mb-4 fade-in">
    <form method="GET" class="flex flex-wrap gap-2 items-center">
        ...
        @endif
        @if(request()->hasAny(['search','kategori_id','tahun']))
        <a href="{{ route('guru.arsip.index') }}" class="text-xs text-gray-400 hover:text-gray-600 self-center">Reset</a>
        @endif
    </form>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden fade-in">
    ...
    <div class="py-14 text-center">
        ...
        <p class="text-xs text-gray-400 mb-3">Coba ubah kata kunci atau filter kategori/tahun</p>
        <a href="{{ route('guru.arsip.index') }}" class="text-xs text-primary-700 hover:underline">Reset filter</a>
        @else
        <p class="text-sm font-medium text-gray-500 mb-1">Belum ada arsip</p>
        <p class="text-xs text-gray-400 mb-3">Dokumen yang Anda unggah akan muncul di sini</p>
        <a href="{{ route('guru.arsip.create') }}" class="inline-flex items-center gap-1.5 btn-xs btn-primary-xs">Tambah Arsip Pertama</a>
        @endif
    </div>
    @else
            ...
            <tr>
                ...
                <td style="text-align:right">
                    <div class="flex items-center justify-end gap-1">
                        ...
                        <a href="{{ route('guru.arsip.edit', $a->id) }}" class="btn-icon" title="Edit">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </a>
                        <form action="{{ route('guru.arsip.destroy', $a->id) }}" method="POST" data-confirm="Hapus arsip ini?" data-confirm-btn="Ya, Hapus" class="inline">
                            @csrf @method('DELETE')
    ...
    {{-- Mobile: card list — 1 arsip = 1 card, aksi ditata jadi tombol besar untuk jempol --}}
    <div class="md:hidden divide-y divide-gray-50">
        ...
        <div class="p-4">
            ...
            <div class="flex items-center gap-2 mt-3">
                ...
                <a href="{{ route('guru.arsip.edit', $a->id) }}"
                ...
                <form action="{{ route('guru.arsip.destroy', $a->id) }}" method="POST" data-confirm="Hapus arsip ini?" data-confirm-btn="Ya, Hapus" class="flex-shrink-0">
                    @csrf @method('DELETE')
