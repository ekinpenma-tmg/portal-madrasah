...
<div class="max-w-2xl mx-auto">
    <a href="{{ route('madrasah.pengajuan.riwayat') }}" class="inline-flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 mb-3">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        Kembali ke riwayat
    </a>
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        @php $isD=$pengajuan->status==='diterima'; $isT=$pengajuan->status==='ditolak'; $bg=$isD?'bg-green-700':($isT?'bg-red-600':'bg-amber-500'); @endphp
