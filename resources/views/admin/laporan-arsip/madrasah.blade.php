...
<div class="flex gap-1 mb-4 fade-in">
    <a href="{{ route('admin.laporan-arsip.guru') }}" class="px-4 py-2 rounded-lg text-xs font-semibold text-zinc-500 hover:bg-zinc-100 transition">Guru</a>
    <a href="{{ route('admin.laporan-arsip.madrasah') }}" class="px-4 py-2 rounded-lg text-xs font-semibold bg-zinc-900 text-white">Madrasah</a>
</div>

{{-- Filter --}}
<div class="bg-white rounded-xl border border-zinc-100 px-4 py-3 mb-4 fade-in">
    <form method="GET" class="flex flex-wrap gap-2 items-center">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari NSM/nama madrasah..."
        ...
        @if(request()->hasAny(['search','hanya_belum_lengkap']))
        <a href="{{ route('admin.laporan-arsip.madrasah') }}" class="text-xs text-zinc-400 hover:text-zinc-600">Reset</a>
        @endif
    </form>
</div>
...
<div class="bg-white rounded-xl border border-zinc-100 overflow-hidden fade-in">
    ...
    <div class="py-14 text-center">
        ...
        <p class="text-sm text-zinc-400">{{ request('hanya_belum_lengkap') ? 'Semua madrasah di halaman ini sudah lengkap dokumennya.' : 'Tidak ada madrasah sesuai filter ini.' }}</p>
    </div>
    @else
        ...
        <table class="w-full">
            <thead>
                <tr class="tbl-head">
                    ...
                    <th class="px-4 py-2.5 text-left text-2xs font-semibold text-zinc-600 uppercase tracking-wider hidden md:table-cell">NSM</th>
            ...
            <tbody>
                ...
                <tr class="tbl-row" style="border-bottom:1px solid #fafafa">
                    <td class="px-4 py-3">
                        <p class="text-xs font-medium text-zinc-800">{{ $r->pemilik->madrasah->nama_madrasah ?? '—' }}</p>
                        <p class="text-2xs text-zinc-400">{{ $r->pemilik->madrasah->jenjang ?? '' }}</p>
                    </td>
                    <td class="px-4 py-3 hidden md:table-cell"><code class="text-2xs bg-zinc-100 text-zinc-600 px-1.5 py-0.5 rounded font-mono">{{ $r->pemilik->nsm }}</code></td>
    ...
    <div class="px-4 py-3">{{ $madrasahPage->withQueryString()->links() }}</div>
    @endif
</div>
